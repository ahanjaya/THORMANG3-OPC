...
bool QNodeThor3::init()
{
  ...
  joint_torque_disable_pub_   = nh.advertise<robotis_controller_msgs::SyncWriteItem>("/robotis/sync_write_item", 1);
...
// move to stand pose : base module
void QNodeThor3::moveStandPose()
{
  std_msgs::String init_msg;
  init_msg.data = "stand_pose";

  init_pose_pub_.publish(init_msg);

  log(Info, "Go to robot stand pose.");
}

void QNodeThor3::disableTorque()
{
  std::map<int, std::string>::iterator map_it;
  int count = 0;
  ...
  for (map_it = id_joint_table_.begin(); map_it != id_joint_table_.end(); ++map_it, count++)
  {
    // id = map_it->first;
    // joint_name = map_it->second;
    // ROS_INFO("id : %d, \t Joint Name: %s ", id, joint_name.c_str());
    ...
    sync_write_msg.item_name = "torque_enable";
    sync_write_msg.value.push_back(0);
    sync_write_msg.joint_name.push_back(joint_name);
    joint_torque_disable_pub_.publish(sync_write_msg);
    ros::Duration(0.1).sleep();
  }

  log(Info, "Disable all torque");
}
