...
config_path = rospack.get_path("pioneer_simulation") + "/scripts/placement_DQN/"
...
class Env(gym.Env):
    ...
    def __init__(self):
        ...
        self.pos_y_object      = 0
        self.ori_object        = 0
        self.left_arm_speed    = 0
        self.right_arm_speed   = 0
        
        self.left_arm_trajectory_x    = []
        self.left_arm_trajectory_y    = []
        self.right_arm_trajectory_x   = []
        self.right_arm_trajectory_y   = []
        ...
        high1 = np.array([ self.pos_x_object, self.pos_y_object, self.ori_object])

        self.action_space_robot      = spaces.MultiDiscrete([1, 2, 3, 4]) # @@@@@@@
        self.observation_space_robot = spaces.Box(-high1, high1, dtype=np.float32)
        ...
        self.Desired_pos_y = int(self.screen_height // 4.0 - 20)  # Final pos cm
    ...
    def step1(self, action, fail):
        ...
        if action['LeftHandAct'] == 0:
            ...
            self.left_arm_speed = self.left_arm_speed - step_size
        ...
        if action['RightHandAct'] == 1:
            self.right_arm_speed = self.right_arm_speed - step_size
        elif action['RightHandAct'] == 2:
            self.right_arm_speed = self.right_arm_speed + step_size

        # -----ori_obj, self.pos_x_object, self.pos_y_object = state_obj----
        state_obj      = self.state_obj
        rotation_force = (self.left_arm_speed - self.right_arm_speed)
    ...
    def reset1(self, randomness=False, **keyboard_position):
        self.steps_beyond_done_obj = None
        self.step_count = 0
        safe_zone       = 15
        ...
        if(randomness):
            self.pos_x_object = random.uniform(
                int((self.screen_width // 2) - (self.workSpaceWidth // 2)) + safe_zone,
                int((self.workSpaceWidth // 2) + (self.screen_width // 2)) - safe_zone)

            self.pos_y_object = random.uniform(
                int((self.screen_height // 2) - (self.workSpaceHeight // 2) + (self.objheight // 2)) + safe_zone,
                int((self.workSpaceHeight // 2) + (self.screen_height // 2) - (self.objheight // 2)) - safe_zone)

            self.ori_object = random.uniform(-self.rotation_limitation, self.rotation_limitation)
        else:
    ...
    def render1(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            # ------------------------
            # WorkSpace
            l, r, t, b = -self.workSpaceWidth, self.workSpaceWidth, self.workSpaceHeight, - self.workSpaceHeight
            workspace  = rendering.make_polygon([(l, b), (l, t), (r, t), (r, b)], False)
            workspace.set_color(.0, .0, .0)
            self.workspacetrans = rendering.Transform(translation=(self.screen_width/2.0, self.screen_height/3.0 - 20), rotation=0.00)
            ...
            self.zonetrans = rendering.Transform(translation=(self.Desired_pos_x, self.Desired_pos_y), rotation=-0.00)
            zone.add_attr(self.zonetrans)
            self.viewer.add_geom(zone)

            # left Arm
            larm = rendering.Image(config_path + 'Environment/larm.jpg', self.armwidth, self.armheight)
            ...
            # right Arm
            rarm = rendering.Image(config_path + 'Environment/rarm.jpg', self.armwidth, self.armheight)
            ...
            # keyboard
            obj1 = rendering.Image(config_path + 'Environment/Object_Keyboard.jpg', self.objwidth, self.objheight)
        ...
        # objx = x[1] * stepsize + self.screen_width // 2.0  
        # objy = x[2] * stepsize + self.screen_height // 4.0  

        pos = self.movementPath[self.step_count]

        self.objtrans.set_translation(pos[0], pos[1])
        disR = math.tanh(np.radians((orientation_state / self.rotation_step_resolution))) * (self.objwidth / 2)
        
        self.larmtrans.set_translation( pos[0] - self.objwidth // 2 - 20, pos[1] - self.objheight - 20 - disR)
        self.rarmtrans.set_translation( pos[0] + self.objwidth // 2 + 20, pos[1] - self.objheight - 20 + disR)

        self.left_arm_trajectory_x.append(int(pos[0] - self.objwidth // 2 - 20 + self.armwidth // 3))
        self.left_arm_trajectory_y.append(pos[1] - int(disR - self.armheight // 10))

        self.right_arm_trajectory_x.append(int(pos[0] + self.objwidth // 2 + 20 - self.armwidth // 3))
        self.right_arm_trajectory_y.append(pos[1] + int(disR + self.armheight // 10))

        self.Mark1trans.set_translation(self.left_arm_trajectory_x[self.step_count],  self.left_arm_trajectory_y[self.step_count])
        self.Mark2trans.set_translation(self.right_arm_trajectory_x[self.step_count], self.right_arm_trajectory_y[self.step_count])

        #print(self.LeftArmPath)

        self.pos_x_object = pos[0]
        self.pos_y_object = pos[1]
        self.movement_distance = math.sqrt(math.pow(self.pos_x_object - self.movementPath[self.step_count - 1][0], 2) + \
                                 math.pow(self.pos_y_object - self.movementPath[self.step_count - 1][1], 2))
        # print(self.movement_distance)

        self.objtrans.set_rotation( np.radians(orientation_state * self.rotation_step) )
        # self.larmtrans.set_rotation( np.radians(orientation_state * self.rotation_step) )
