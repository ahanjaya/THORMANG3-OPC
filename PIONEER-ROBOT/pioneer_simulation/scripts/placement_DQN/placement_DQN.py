...
bridge = ros_bridge.Placement_Simulation()
...
def get_screen():
    ...
    if recording_frames:
        frame_counter += 1
        cv2.imwrite(config_path + "frames/" + str(frame_counter) + ".jpg", screen)
    ...
    # resize it to the desired one, themn we have to scale ior to the tensor size
...
eps_threshold_list = []
episode_results    = []
checkpoint_path    = config_path + 'Network/target_net.ckpt'
...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            action_dic = actions_map(action_number)
            #print(action_dic)

            return best_action, action_dic
    else: # Random
        # return {'PathPattern': 0, 'LeftHandAct': 2, 'RightHandAct': 1}
        # return actions_map(random.randrange(n_actions))
        ...
        action_dic = actions_map(random_action)
...
while not bridge.shutdown:
    while not bridge.received:
    ...
    env.reset1(randomness=False, **bridge.keyboard_position)
    ...
    # print(len(env.left_arm_trajectory_x), env.step_count)

    # sending trajectory for left & right arm trajectory
    left_arm_points  = bridge.pack_points('left_arm',  env.step_count, env.left_arm_trajectory_x, env.left_arm_trajectory_y)
    right_arm_points = bridge.pack_points('right_arm', env.step_count, env.right_arm_trajectory_x, env.right_arm_trajectory_y)
    bridge.send_trajectory(left_arm_points, right_arm_points)

    # sending final pose of keyboard simulation
    final_keyboard_pos          = {'x': None, 'y': None, 'theta': None}
    final_keyboard_pos['x']     = env.pos_x_object
    final_keyboard_pos['y']     = env.screen_height - env.pos_y_object
    final_keyboard_pos['theta'] = env.ori_object / env.rotation_step_resolution
    bridge.send_final_pose(**final_keyboard_pos)

    if save_data:
        counter = len(os.walk(data_path + "picts_simulation_finish").__next__()[2])
        counter += 1

        while not bridge.save_frame:
            if bridge.shutdown: break
            else:               pass
    ...
    #     plt.savefig(config_path + 'Results/' + 'experiment'+ str(experiment_Number) +'.eps', format='eps')
    #     np.save(config_path + 'Results/Output' + str(experiment_Number) +'.npy', final_error)
