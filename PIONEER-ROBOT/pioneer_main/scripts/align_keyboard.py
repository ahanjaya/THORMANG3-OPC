...
align_keyboard_path  = rospack.get_path("pioneer_main") + "/config/thormang3_align_keyboard_ws.yaml"
...
def grasp_key_callback(msg):
    global grasp_keyboard
    grasp_keyboard = msg.data
...
def load_config(arm):
    try:
        with open(align_keyboard_path, 'r') as f:
...
def move_arm(kinematics, arm, x, y):
    ...
    if arm == "left_arm":
        kinematics.set_kinematics_pose(arm , 2.0, **{ 'x': x, 'y': y, 'z': 0.65, 'roll': -170.00, 'pitch': 20.00, 'yaw': -40.00 })
        prev_lik = (x, y)
    elif arm == "right_arm":
        kinematics.set_kinematics_pose(arm , 2.0, **{ 'x': x, 'y': y, 'z': 0.64, 'roll': 170.00, 'pitch': 20.00, 'yaw': 40.00 })
        prev_rik = (x, y)
...
def main():
    ...
    rospy.Subscriber("/pioneer/target/grasp_keyboard", Bool,    grasp_key_callback)
    rospy.Subscriber("/pioneer/align_keyboard/init_pose", Bool, ini_pose_callback)
    ...
    lik_ymin_lower = 0.08 #0.1
    lik_ymax_lower = 0.28 #0.24
    lik_ymin_upper = 0.01 #0.05
    lik_ymax_upper = 0.35 #0.34
    lik_xmin       = 0.14 #0.15
    lik_xmax       = 0.45 #0.45
    ...
    rik_ymin_lower = -0.08 #-0.1
    rik_ymax_lower = -0.28 #-0.24
    rik_ymin_upper = -0.01  #-0.05
    rik_ymax_upper = -0.35 #-0.34
    rik_xmin       = 0.14  #0.15
    rik_xmax       = 0.45  #0.45
    ...
    if scan_workspace:
        ...
        for idx, pts in enumerate(left_arm_pts):
            ...
            kinematics.set_kinematics_pose("left_arm" , 2.0, **{ 'x': x, 'y': y, 'z': 0.65, 'roll': -170.00, 'pitch': 20.00, 'yaw': -40.00 })
        ...
        for idx, pts in enumerate(right_arm_pts):
            ...
            kinematics.set_kinematics_pose("right_arm" , 2.0, **{ 'x': x, 'y': y, 'z': 0.64, 'roll': 170.00, 'pitch': 20.00, 'yaw': 40.00 })
        ...
        if save_config:
            ...
            with open(align_keyboard_path, 'w') as f:
    ...
    while not rospy.is_shutdown():
        ...
        if init_pose:
            if prev_lik and prev_rik:
                lx_ik = prev_lik[0]
                ly_ik = prev_lik[1] + 0.05
                move_arm(kinematics, "left_arm" , lx_ik, ly_ik)
                rx_ik = prev_rik[0]
                ry_ik = prev_rik[1] - 0.05
        ...
        if start_ik == True: # Input by Middle button Mouse
            rospy.loginfo("Robot Arm Start Moving")
        ...
        if grasp_keyboard == True:
            lx_ik = prev_lik[0]
            ly_ik = prev_lik[1] - 0.065
            move_arm(kinematics, "left_arm" , lx_ik, ly_ik)

            rx_ik = prev_rik[0]
            ry_ik = prev_rik[1] + 0.065
            move_arm(kinematics, "right_arm" , rx_ik, ry_ik)
            grasp_keyboard = False
